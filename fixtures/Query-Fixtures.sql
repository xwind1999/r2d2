...
-- partners
SELECT p.golden_id as id,  p.status as type, p.currency as currencyCode, ifnull(p.cease_date, '') as partnerCeaseDate, ifnull(p.updated_at, '') as updatedAt FROM partner p WHERE FIND_IN_SET(p.golden_id, @partners COLLATE utf8mb4_general_ci);
